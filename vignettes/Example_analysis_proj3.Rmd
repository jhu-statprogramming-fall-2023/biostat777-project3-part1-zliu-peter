...
library(rio)
#library(plotly)
#for some reason plotly does not work when building github pages
library(tidyverse)
library(magrittr)
library(lubridate)
library(tidyr)
...
budget_revenue_plot <- movies_Q3 %>%
    ...
    geom_point(aes(fill=Popularity, color=Popularity, alpha=Popularity), pch=21, size=2.5) +
    scale_y_log10() +
    scale_x_log10() +
    geom_smooth(method = "lm", se = FALSE, color = "black") +
    labs(x = "Budget (in USD)", y = "Revenue (in USD)",
         title = "Budget vs Revenue of Horror Movies",
         subtitle = "Higher budget seems roughly correlated with higher revenue.",
         caption = "log10-scaled x, y axis.") +
    theme_classic()+
...
budget_revenue_plot <- budget_revenue_plot +
    scale_color_manual(values = c(`Top 10` = "red", "Others" = "grey70")) +
    scale_fill_manual(values = c(`Top 10` = "red", "Others" = NA)) +
    scale_alpha_manual(values = c(`Top 10` = 1, "Others" = 0.3))

budget_revenue_plot


# # plotting: original plan was to use plotly. It worked locally but for some reasons cannot work with github pages.
# budget_revenue_plot <- movies_Q3 %>%
#     ggplot(aes(x = budget, y = revenue)) +
...
# 
# budget_revenue_plot <- budget_revenue_plot +
#     scale_color_manual(values = c(`Top 10` = "red", "Others" = "grey95")) +
#     scale_fill_manual(values = c(`Top 10` = "red", "Others" = NA)) +
...
We can see that higher budget seems to be associated with higher revenue, though the exact relationship needs to be further investigated. We also see that most of the top 10 popular movies are those high in both production cost and revenue. This suggests that higher revenue might be correlated with higher popularity.
